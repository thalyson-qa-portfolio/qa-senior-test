...
- **Três jobs em paralelo:**
  ...
  2. **e2e-tests** — `npm ci`, Playwright Chromium, `npm run test:e2e` (Cucumber + geração do dashboard `multiple-cucumber-html-reporter`) → artifact **e2e-report** com `test-output/reports/` (Cucumber HTML + JSON) e `test-output/cucumber-html-report/` (dashboard com pizzas e features). Se a suíte falhar, artifact **e2e-failure-evidence** (traces, screenshots, videos).

...
- **Três jobs em paralelo:**
  ...
  2. **e2e-tests** — `npm ci`, Playwright Chromium, Java 17 (para Allure), `npm run test:e2e`, geração `npx allure generate` → artifact **e2e-report** com `test-output/reports/` (Cucumber HTML + JSON) e `test-output/allure-report/` (Allure: pizza, feature → cenário → steps). Se a suíte falhar, artifact **e2e-failure-evidence** (traces, screenshots, videos).
